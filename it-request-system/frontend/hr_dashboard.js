...
function updateStats(requests) {
  const stats = {
    ...
    pending: requests.filter((r) => r.status.hr === "pending").length,
    approved: requests.filter((r) => r.status.hr === "approved").length,
    rejected: requests.filter((r) => r.status.hr === "rejected").length,
  };
...
function displayRequests(requests, filter = "all") {
  ...
  const filteredRequests =
    filter === "all"
      ? requests
      : requests.filter((r) => r.status && r.status.hr === filter);
  ...
  filteredRequests.forEach((request) => {
    ...
    // Determine if HR buttons should be disabled
    const isHODApproved = request.status.hod === "approved";
    const isHRPending = request.status.hr === "pending";
    const showHRActions = isHRPending && isHODApproved;
    const disableHRActions = isHRPending && !isHODApproved;
    ...
    tr.innerHTML = `
            ...
            <td>
                ${
                  request.status.hr === "pending" && !request.actionedViaEmail?.hr
                    ? `
                    <button class="btn btn-sm btn-outline-success action-btn approve" 
                            data-id="${request._id}" 
                            data-role="hr"
                            ${disableHRActions ? 'disabled' : ''}>
                        <i class="fas fa-check"></i>
                        ${disableHRActions ? ' (Awaiting HOD)' : ''}
                    </button>
                    ...
                    ${
                      disableHRActions 
                        ? '<div class="small text-muted mt-1">Requires HOD approval first</div>' 
                        : ''
                    }
                `
                    : request.actionedViaEmail?.hr
...
function handleFilter(e) {
  ...
  const role = btn.dataset.role || "hr";
  const requests = JSON.parse(localStorage.getItem("requests") || "[]");
